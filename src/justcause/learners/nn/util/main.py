...
def train_dragon(t, y_unscaled, x, targeted_regularization=True,
                knob_loss=dragonnet_loss_binarycross, ratio=1., val_split=0.1, batch_size=512, num_epochs=100, learning_rate=0.001, verbose=1):
    ...
    dragonnet.compile(optimizer=Adam(learning_rate = learning_rate), loss=loss, metrics=metrics)

    class EarlyStoppingByLossVal():
        def __init__(self, Callback, monitor="regression_loss", value=400, verbose=0):
...
def train_and_predict_dragons(
    ...
    for i in range(runs):
        ...
        dragonnet.compile(optimizer=Adam(learning_rate = 1e-3), loss=loss, metrics=metrics)
...
def train_and_predict_ned(
    ...
    for i in range(runs):
        ...
        nednet.compile(optimizer=Adam(learning_rate = 1e-3), loss=ned_loss, metrics=metrics_ned)
        ...
        cut_net.compile(optimizer=Adam(learning_rate = 1e-3), loss=dead_loss, metrics=metrics_cut)
